//===- llvm/Support/OutputBackend.h - Output management ---------*- C++ -*-===//
//
...
public:
  Expected<std::unique_ptr<OutputFile>> createFile(const Twine &Path,
                                                   OutputConfig Config = None);
  ...
  createUniqueFile(const Twine &Model, OutputConfig Config = None);
  ...
  Expected<IntrusiveRefCntPtr<OutputDirectory>>
  createUniqueDirectory(const Twine &Prefix, OutputConfig Config = None);
...
public:
  ...
  struct OutputSettings {
    ...
    /// for outputs with \a OutputConfigFlag::NoCrashCleanup.
    ...
    struct {
      ...
      // outputs with \a OutputConfigFlag::Volatile.
      bool Forward = true;
    } WriteThrough;
  };
