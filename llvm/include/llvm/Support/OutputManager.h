...
public:
  ...
  /// Constructor for an output. \p Dest must be a newly initialized \a
  /// OutputDestination, or \c nullptr.
  ///
  /// \post \a getOS() returns a pointer to the stream for writing to \p Dest
  /// (\a raw_null_ostream if \p Dest is \c nullptr). This may write to \a
  /// Bytes as an intermediate buffer.
...
public:
  ...
  bool shouldRejectExistingFiles() const { return ShouldRejectExistingFiles; }
  void setShouldRejectExistingFiles(bool RejectExisting) {
    ShouldRejectExistingFiles = RejectExisting;
  }
...
private:
  bool ShouldRejectExistingFiles = false;
