...
static Expected<T> createStableUniqueEntity(
  ...
  Config.set(OutputConfigFlag::NoOverwrite);
  int NumTries = 0;
...
private:
  ...
  /// - Unless \a OutputConfigFlag::NoAtomicWrite, a temporary file is opened
  ...
  if (Config.get(OutputConfigFlag::AtomicWrite)) {
    ...
    if (sys::fs::exists(Status)) {
      if (!sys::fs::is_regular_file(Status))
        Config.set(OutputConfigFlag::NoAtomicWrite);
...
InMemoryOutputBackend::createFileImpl(StringRef ResolvedPath,
  ...
  // FIXME: OutputConfigFlag::NoOverwrite also needs to check for in-flight
  // outputs.
