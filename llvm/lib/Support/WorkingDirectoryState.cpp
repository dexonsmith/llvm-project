...
static void makeRootImpl(Optional<char> Drive, SmallVectorImpl<char> &Path,
                         Style PathStyle) {
  const StringRef Separator = get_separator(PathStyle);
  assert(Separator.size() == 1);
  assert(Separator[0] == '/' || Separator[0] == '\\');

  StringRef PathRef(Path.data(), Path.size());
  SmallString<256> AbsPath;
  if (!is_style_posix(PathStyle)) {
    AbsPath.push_back(Separator);
  } else if (Drive) {
    AbsPath.push_back(*Drive);
    AbsPath.append(":");
    AbsPath.append(Separator);
  } else if (PathRef.size() >= 3 && is_separator(PathRef[0], PathStyle) &&
...
bool WorkingDirectoryState::isAbsolute(const Twine &Path_, Style PathStyle) {
  ...
  assert(is_style_windows(PathStyle));
  if (Path.size() < 3)
    return false;
  ...
  case ':':
    ...
    return is_separator(Path[2], PathStyle) &&
           ((Path[0] >= 'a' && Path[0] <= 'z') ||
            (Path[0] >= 'A' && Path[0] <= 'Z'));
  case '/':
  case '\\':
...
WorkingDirectoryState::getDriveWorkingDirectory(char Drive) const {
  assert(is_style_windows(getPathStyle()));
