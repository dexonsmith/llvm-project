...
void Output::erase() {
  // Release resources in destruction order to erase.
  assert(isOpen() && "Output already closed or erased");
  IsOpen = false;
...
OutputManager::OutputManager()
    : OutputManager(makeIntrusiveRefCnt<OnDiskOutputBackend>()) {}
...
public:
  ...
  std::unique_ptr<raw_pwrite_stream> takeStreamImpl() override;
...
OnDiskOutputDestination::convertMappedFileToBuffer(
    std::unique_ptr<sys::fs::mapped_file_region> Mapping) {
  ...
  public:
    StringRef getBufferIdentifier() const override { return OutputPath; }
    BufferKind getBufferKind() const override { return MemoryBuffer_MMap; }

    MappedMemoryBuffer(const std::string &OutputPath,
                       std::unique_ptr<sys::fs::mapped_file_region> Mapping)
        : OutputPath(OutputPath), Mapping(std::move(Mapping)) {
  ...
  return std::make_unique<MappedMemoryBuffer>(OutputPath, std::move(Mapping));
}

Expected<std::unique_ptr<sys::fs::mapped_file_region>>
OnDiskOutputDestination::openMappedFile(size_t Size) {
...
Error OnDiskOutputDestination::closeFile(StringRef OutputPath,
  ...
  if (useWriteThroughBuffer()) {
    ...
    if (MaybeContent &&
        Config.test(OnDiskOutputConfig::ForwardWriteThroughBuffer) &&
        BufferedContent->size() >=
...
public:
  ...
  Error storeContentImpl(ContentBuffer &Content) override;
...
InMemoryOutputBackend::createDestination(
    ...
    std::unique_ptr<OutputDestination> NextDest) {
  if (shouldRejectExistingFiles() && InMemoryFS->exists(OutputPath))
