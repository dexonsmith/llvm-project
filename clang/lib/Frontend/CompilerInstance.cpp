...
void CompilerInstance::setOutputBackend(
    IntrusiveRefCntPtr<llvm::vfs::OutputBackend> Value) {
  assert(!OutputBackend && "Already has an output manager");
  OutputBackend = std::move(Value);
}

void CompilerInstance::createOutputBackend() {
  setOutputBackend(llvm::makeIntrusiveRefCnt<llvm::vfs::OnDiskOutputBackend>());
}

llvm::vfs::OutputBackend &CompilerInstance::getOutputBackend() {
  assert(OutputBackend);
...
CompilerInstance::createOutputFileImpl(StringRef OutputPath, bool Binary,
  ...
  Expected<OutputFile> O =
      consumeDiscardOnDestroy(getOrCreateOutputBackend().createFile(
...
compileModuleImpl(CompilerInstance &ImportingInstance, SourceLocation ImportLoc,
  ...
  // Share the output backend.
  Instance.setOutputBackend(&ImportingInstance.getOrCreateOutputBackend());
